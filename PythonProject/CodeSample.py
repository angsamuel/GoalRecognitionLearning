def shadowToVisible(edges, shadowNodes, shadowGroups, nodesNum):
	...
	for e in edges:
		#if the edge does not contain a shadow state, add it to the new edges
		if e[0] not in shadowNodes and e[1] not in shadowNodes: 
			newEdges.append(e)
		#if the edge connects a shadow state to a visible state... 
		elif (e[0] in shadowNodes and e[1] not in shadowNodes) or (e[0] not in shadowNodes and e[1] in shadowNodes):
			entryNode = -1
			shadowNode = -1

			#identify which state is the shadow state
			...
			#add edges to respective groups for later use
		...
		elif (e[0] in shadowNodes and e[1] in shadowNodes):
			for m in range(0, len(shadowGroups)):
